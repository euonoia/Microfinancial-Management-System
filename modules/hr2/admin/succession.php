...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_candidate'])) {
    ...
    $employee_id = intval($_POST['employee_id']); // PK id from employees
    $readiness = $_POST['readiness'];
    $development_plan = trim($_POST['development_plan']);

    if ($branch_id !== '' && $employee_id > 0) {
        $stmt = $conn->prepare("
            INSERT INTO successor_candidates (branch_id, employee_id, readiness, development_plan)
            VALUES (?, ?, ?, ?)
        ");
        $stmt->bind_param("siss", $branch_id, $employee_id, $readiness, $development_plan);
        $stmt->execute();
        $stmt->close();
    }
...
SELECT 
    c.*, 
    e.employee_id AS company_id,  -- display varchar company ID
    e.first_name, 
    e.last_name, 
    p.position_title, 
    p.criticality
FROM successor_candidates c
JOIN employees e ON e.id = c.employee_id  -- join on PK
...
$employees = $conn->query("
    SELECT id, employee_id, CONCAT(first_name, ' ', last_name) AS name 
    FROM employees 
        ...
        <form method="POST" style="margin-top:40px;">
            ...
            <select name="employee_id" required>
                ...
                <?php foreach ($employees as $e): ?>
                    <option value="<?= $e['id'] ?>"><?= htmlspecialchars($e['name']) ?> (<?= htmlspecialchars($e['employee_id']) ?>)</option>
                <?php endforeach; ?>
            </select>
